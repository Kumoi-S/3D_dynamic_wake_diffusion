...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        B, N, C = x.shape
        # x: [B,N,C](即[B,T,E])
        # cond: [B,T_cond,E_cond]
        # 如果是图片形式的cond，则可以进行patching，也可以类似unet里做"b c h w -> b (h w) c"的操作(相当于1×1的patching)

        # 投影后，把batch连接在patch_size的维度上，方便并行计算
        q = self.q_linear(x).view(1, -1, self.num_heads, self.head_dim)
        kv: torch.Tensor = self.kv_linear(cond).view(1, -1, 2, self.num_heads, self.head_dim)
        k, v = kv.unbind(2)
...
class T2IFinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, patch_size, out_channels):
        ...
        self.out_channels = out_channels  # 没有意义
